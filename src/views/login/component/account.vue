...
const DragVerifyImgRotate = defineAsyncComponent(
  () => import("/@/components/dragVerify/dragVerifyImgRotate.vue")
);
  ...
  ruleFormRef.value.validate(async (valid: boolean) => {
    ...
    try {
      ...
      const [err, res] = await feature(
        getAPI(SysAuthApi).apiSysAuthLoginPost(state.ruleForm)
      );
      if (err) {
        getCaptcha(); // 重新获取验证码
        return;
